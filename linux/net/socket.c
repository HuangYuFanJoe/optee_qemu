...
int __sys_send_with_pid_to(int fd, void __user *buff, size_t len, unsigned int flags,
		 struct sockaddr __user *addr,  int addr_len, int frompid, int topid)
{
	struct socket *sock;
	struct sockaddr_storage address;
	int err;
...
SYSCALL_DEFINE6(send_with_pid, int, fd, void __user *, buff, size_t, len,
		unsigned int, flags, int, frompid, int, topid)
{	
	return __sys_send_with_pid_to(fd, buff, len, flags, NULL, 0, frompid, topid);
}
